...
import { useDeleteInternalProject, useInternalProjects } from '@/features/projects/hooks'
...
export function InternalProjectsPage() {
  ...
  const { data, isLoading, isError, error } = useInternalProjects(filters)
  const deleteMutation = useDeleteInternalProject()

  const handleDelete = async (item: ProjectListItem) => {
    if (!confirmProjectDelete(item.name, 'internal-projects')) {
      return
    }
    try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Manage internal product, capability, governance and improvement initiatives."
        actions={
          <PermissionGate require="editor">
            <Link to="/internal-projects/new">
              <Button>New Internal Project</Button>
            </Link>
          </PermissionGate>
        }
      />
      ...
      {isError ? (
        ...
        <>
          <ProjectTable
            ...
            variant="internal-projects"
            isLoading={isLoading}
            onOpen={(id) => navigate(`/internal-projects/${id}`)}
            onEdit={(id) => navigate(`/internal-projects/${id}/edit`)}
            onDelete={handleDelete}
          />

          {!isLoading && (data?.items.length ?? 0) === 0 ? (
            <EmptyState
              title="No internal projects yet"
              description="Create your first internal project to track platform, capability and improvement work."
              action={
                <PermissionGate require="editor">
                  <Link to="/internal-projects/new">
                    <Button>New Internal Project</Button>
